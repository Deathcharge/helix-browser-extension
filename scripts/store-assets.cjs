...
const promoTile = {
  ...
  body: 'Private source triage, on device.'
};
...
function html(asset) {
  ...
  </style></head><body><main class="canvas"><section class="copy"><div class="brand"><span class="mark">◎</span><span>Samsarix Page Lens</span></div><div class="eyebrow">${asset.eyebrow}</div><h1 class="title">${asset.title}</h1><p class="body">${asset.body}</p><div class="badges">${asset.badges.map(value => `<span class="badge">${value}</span>`).join('')}</div></section><section class="visual"><div class="${asset.mode}"><img src="${asset.image}" alt="Verified Samsarix Page Lens interface"></div><span class="local">VERIFIED ${version} INTERFACE</span></section></main></body></html>`;
}

function promoHtml(asset) {
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    *{box-sizing:border-box}html,body{margin:0;width:440px;height:280px;overflow:hidden}body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;color:#f7f5ff;background:#0b0e1c}.tile{position:relative;width:100%;height:100%;padding:28px 30px;background:radial-gradient(circle at 100% 0,#49378f 0,#1c1836 36%,#0b0e1c 75%)}.tile:after{content:"◎";position:absolute;right:24px;bottom:13px;color:#8f7ee8;font-size:74px;opacity:.36}.brand{display:flex;align-items:center;gap:8px;color:#dcd9ed;font-size:13px;font-weight:760}.mark{color:#a392ff;font-size:19px}.title{max-width:355px;margin:35px 0 13px;font-size:29px;line-height:1.07;letter-spacing:-.035em;white-space:pre-line}.body{margin:0;color:#8ee3be;font-size:13px;font-weight:720;letter-spacing:.025em}
  </style></head><body><main class="tile"><div class="brand"><span class="mark">◎</span><span>Samsarix Page Lens</span></div><h1 class="title">${asset.title}</h1><p class="body">${asset.body}</p></main></body></html>`;
}
...
(async () => {
  ...
  try {
    ...
    await page.setContent(promoHtml(promoTile), { waitUntil: 'load' });
    await page.screenshot({ path: path.join(outputDir, promoTile.file), type: 'png' });
  ...
  console.log(`Built ${assets.length} Chrome Web Store screenshots and one small promotional tile in store-assets/.`);
