...
(async () => {
  ...
  try {
    ...
    await popup.evaluate(() => save()); await popup.getByText('Recent saved briefs').waitFor(); await popup.getByRole('button', { name: 'Open saved brief: Research Signals' }).click(); await popup.getByText('Source signals').first().waitFor();
