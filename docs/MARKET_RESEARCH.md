...
5. **Decision implemented in 1.6:** make the local queue explicitly portable through a small, versioned JSON contract and Markdown handoff. [Chrome documents `storage.local`](https://developer.chrome.com/docs/extensions/reference/api/storage) as device-local, limited to 10 MB, and removed with the extension; Page Lens remains well below that quota without requesting `unlimitedStorage`. [Zotero’s current export guidance](https://www.zotero.org/support/kb/exporting) reinforces that export and backup are distinct concepts, so Page Lens labels its exact queue JSON as a Page Lens backup while describing Markdown as handoff—not lossless restore or cloud sync.
