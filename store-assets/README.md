...
The generator uses no remote fonts or imagery. The embedded product UI comes from the packaged 1.7 browser journey; surrounding copy is maintained in `scripts/store-assets.cjs`. Before submission, the store owner must review every image against the exact uploaded ZIP and current Chrome Web Store image/content policies.
