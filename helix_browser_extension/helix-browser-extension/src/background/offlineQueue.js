 ...
 *
 * Security note: Authorization headers are NOT persisted to chrome.storage.local.
 * They are stripped at enqueue time and re-injected from the live session token
 * at process time, preventing token leakage on browser crash or storage inspection.
 */
...
// Headers that must never be persisted to disk (contain credentials)
...
class OfflineQueue {
   ...
   * Sensitive headers (Authorization, x-api-key, etc.) are stripped before
   * persisting so tokens are never written to chrome.storage.local.
   */
  async enqueue(request) {
    ...
    const queueItem = {
      ...
      headers: safeHeaders, // auth headers intentionally excluded
      body: request.body,
    ...
    for (const item of sorted) {
      try {
        ...
        if (item.retryCount >= MAX_RETRIES) {
          ...
          console.log(
            `[OfflineQueue] Retry ${item.retryCount}/${MAX_RETRIES} in ${delay}ms`
          );
   ...
   * Re-injects the current session auth token at replay time so tokens are
   * never stored in persistent local storage.
   */
  async processItem(item) {
    const headers = { ...item.headers };

    // Re-hydrate auth header from current session (not from stored queue)
