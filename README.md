...
Extraction visits at most 15,000 DOM nodes and retains at most 250,000 article-text characters. URLs, titles, metadata, outlines, and source labels have separate field caps before the snapshot leaves the page. It excludes content inside scripts, styles, navigation, footers, forms, dialogs, templates, and elements explicitly marked hidden or `aria-hidden`, including excluded primary roots and metadata. It does not inspect form values. CSS-only visibility cannot be inferred without substantially more page work, so the privacy disclosure does not claim that all CSS-hidden text is excluded.
...
- [Security review and hardening record](docs/SECURITY_REVIEW.md)
